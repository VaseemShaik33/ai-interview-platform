...
public class InterviewSessionService {
        ...
        public SubmitAnswerResponse submitAnswer(
                ...
                if (currentQuestionNumber == session.getTotalQuestions()) {
                        ...
                        return new SubmitAnswerResponse(
                                        ...
                                        null,
                                        new com.aiinterview.platform.dto.EvaluationResult(
                                                evaluation.score(), evaluation.feedback(), evaluation.correctness().name()));
                }
        ...
        public com.aiinterview.platform.dto.InterviewResultDetailResponse getDetailedResult(Long sessionId, User user) {
                InterviewSession session = interviewSessionRepository.findById(sessionId)
                                .orElseThrow(() -> new RuntimeException("Interview session not found"));
                verifySessionOwner(session, user);

                List<InterviewAnswer> answers = interviewAnswerRepository.findBySessionId(sessionId);
                long totalScore = answers.stream().mapToLong(a -> a.getScore() == null ? 0 : a.getScore()).sum();
                long maximumScore = session.getTotalQuestions() * 10L;
                double percentage = maximumScore == 0 ? 0 : totalScore * 100.0 / maximumScore;

                List<com.aiinterview.platform.dto.AnswerFeedbackResponse> feedback = answers.stream().map(a ->
                        new com.aiinterview.platform.dto.AnswerFeedbackResponse(
                                a.getQuestion().getId(), a.getQuestion().getQuestionText(), a.getUserAnswer(),
                                a.getScore(), a.getCorrectness() == null ? "UNKNOWN" : a.getCorrectness().name(), a.getFeedback())
                ).toList();

                long correct = answers.stream().filter(a -> a.getCorrectness() == Correctness.CORRECT).count();
                long partial = answers.stream().filter(a -> a.getCorrectness() == Correctness.PARTIAL).count();
                long incorrect = answers.stream().filter(a -> a.getCorrectness() == Correctness.INCORRECT).count();

                List<String> strengths = new java.util.ArrayList<>();
                List<String> areas = new java.util.ArrayList<>();
                if (correct > 0) strengths.add("Good understanding of core concepts");
                if (percentage >= 80) strengths.add("Strong overall interview performance");
                if (correct == 0) strengths.add("You completed the interview and can build from this attempt");
                if (incorrect > 0) areas.add("Review concepts behind incorrect answers");
                if (partial > 0) areas.add("Add more depth and examples to partially correct answers");
                if (percentage < 70) areas.add("Practice more questions before your next mock interview");

                return new com.aiinterview.platform.dto.InterviewResultDetailResponse(
                        session.getId(), session.getCategory().getName(), session.getDifficulty().name(),
                        session.getTotalQuestions(), answers.size(), totalScore, maximumScore, percentage,
                        session.getStatus(), feedback, strengths, areas);
        }
